...
class ConvTerrianClimate(nn.Module):
    def __init__(self, num_channels):
        super(ConvTerrianClimate, self).__init__()
        self.num_channels = num_channels
        self.conv1 = nn.Conv2d(in_channels=self.num_channels, out_channels=16, kernel_size=(2, 2), stride=1, padding=1)
        self.conv2 = nn.Conv2d(in_channels=16, out_channels=16, kernel_size=(2, 2), stride=1, padding=1)
        self.fc1 = nn.Linear(16*3*1, 16)
...
class CovLSTM(nn.Module):
    def __init__(self, cnn_num_channels,
                 lstm_input_size, lstm_hidden_size, lstm_num_layers=2, lstm_dropout=0):
        super(CovLSTM, self).__init__()
        self.conv1 = nn.Conv2d(in_channels=cnn_num_channels, out_channels=16, kernel_size=(2, 2), stride=1, padding=1)
        self.conv2 = nn.Conv2d(in_channels=16, out_channels=16, kernel_size=(2, 2), stride=1, padding=1)
        self.fc_cnn_1 = nn.Linear(16*3*1, 16)
        ...
        nn.init.xavier_uniform_(self.fc_cnn_1.weight)
        nn.init.xavier_uniform_(self.fc_lstm_1.weight)

    def forward(self, x_cnn, x_lstm):
        # x_cnn = x_cnn[:, :, :, :]
        x_lstm = x_lstm.squeeze(dim=1)
        x_cnn = F.max_pool2d(F.relu(self.conv1(x_cnn)), (2, 2))
        x_cnn = F.max_pool2d(F.relu(self.conv2(x_cnn)), (2, 2))
        # print('current size:', x_cnn.shape)
